...
# -*- coding: utf-8 -*-
"""
TXO Dealer Gamma Exposure (GEX) 模型  ─ LINE 推播版
====================================================
每天 08:00 (UTC 00:00) 由 GitHub Actions 自動執行，
產生 GEX 圖後透過 LINE Messaging API push 給自己。

所需 GitHub Secrets:
  LINE_CHANNEL_ID     → OA Manager > Settings > Messaging API > Channel ID
  LINE_CHANNEL_SECRET → OA Manager > Settings > Messaging API > Channel secret
  LINE_USER_ID        → 你自己的 LINE user ID (U 開頭)
  IMGBB_KEY           → imgbb.com 免費 API key (圖片暫存 URL)
"""
...
def build_model(df, spot, today=None):
    ...
    # (2) GEX(S) sweep → macro/micro zero
    ...
    total    = (G * (sign * OI)[None, :] * MULT * S_grid[:, None]**2 * 0.01).sum(axis=1) / 1e8
    total_s  = gaussian_filter1d(total, 4)
    near_T   = df["T"].min()
    ...
    def zero_cross(curve):
        ...
        if not len(idx):
            return None
        xs = [S_grid[i] - curve[i] * (S_grid[i+1] - S_grid[i]) /
              (curve[i+1] - curve[i]) for i in idx]
    ...
    peak_S   = S_grid[np.argmax(speed)]
    valley_S = S_grid[np.argmin(speed)]

    # (4) Call/Put Wall (evaluate gamma at S=K)
...
# ══════════════════════════════════════════════ LINE helpers
def get_line_token():
    """Channel ID + Secret → stateless access token (30天有效,每次自動換)"""
    cid    = os.environ["LINE_CHANNEL_ID"]
...
def upload_imgbb(path):
    """上傳 PNG → imgbb，回傳公開 HTTPS URL (LINE 需要 URL 才能顯示圖片)"""
    ...
    r = requests.post("https://api.imgbb.com/1/upload",
                      params={"key": key},
                      data={"image": data, "expiration": 7200},
                      timeout=30)
    r.raise_for_status()
    return r.json()["data"]["url"]

def push_line(token, user_id, image_url, summary):
    """同時 push 文字摘要 + 圖片"""
    ...
    r.raise_for_status()
    print(f"[LINE] push OK  status={r.status_code}")

def try_push_line(m, png_path):
    """安靜執行 LINE push；缺少環境變數時跳過(本機測試不報錯)"""
    uid = os.environ.get("LINE_USER_ID", "")
    if not uid or not os.environ.get("LINE_CHANNEL_ID"):
        print("[LINE] 環境變數未設定，跳過 push")
        return
    try:
        token     = get_line_token()
        image_url = upload_imgbb(png_path)
        if not image_url:
            print("[LINE] 無 IMGBB_KEY，跳過 push")
            return
        ...
        push_line(token, uid, image_url, summary)
...
def main():
    ...
    if args.demo:
        ...
        try:
            ...
            print(f"[OpenAPI failed: {e}] → fallback 網頁版")
            df = fetch_txo_web(date.today())
        spot = args.spot or fetch_spot_openapi()
        print(f"[spot/TX futures] {spot:.0f}")
